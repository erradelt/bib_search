...
def write_dir_json(path: Path, file, indent_level=1):
    ...
    for index, item in enumerate(entries):
        ...
        if item.is_dir():
            ...
            sub_dirs, sub_files = write_dir_json(item, file, indent_level + 1)
            dir_count += sub_dirs
...
def generate_bibliography(passedpath, dir_name):
    ...
    with open(temp_file_path, "w", encoding="utf-8") as f:
        ...
        dir_count, file_count = write_dir_json(root_path, f, 2)
        f.write("    }\n")
        f.write("}\n")
